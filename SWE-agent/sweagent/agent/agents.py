...
class SkyDefaultAgentConfig(BaseModel):
    """This configuration object specifies the behavior of an mini sandbox agent for SkyRL."""
    pre_check: bool = True
    """Whether to perform pre-checks before agent interaction."""
    max_retries: int = 1
    total_time_limit: int = 300
    """Total time limit for the agent interaction in seconds."""
    step_limit: int = 60
    ...
    type: Literal["skysbdefault"] = "skysbdefault"
...
class DefaultAgentConfig(BaseModel):
    """This configuration object specifies the behavior of an normal swe agent."""
    total_time_limit: int = 300
    """Total time limit for the agent interaction in seconds."""
    reward: bool = True
    """Whether to calculate reward signal from environment."""
    step_limit: int = 60
    """Maximum number of steps the agent can take."""
    test_output: bool = True
    """Whether to record test output during reward calculation. """
...
AgentConfig = Annotated[SandboxAgentConfig | EmptyAgentConfig | SkyDefaultAgentConfig |DefaultAgentConfig | RetryAgentConfig | ShellAgentConfig, Field(union_mode="left_to_right")]
...
class _TotalRunningTimeExceeded(Exception):
    """Used for internal control flow"""
...
class DefaultAgent(AbstractAgent):
    ...
    def handle_submission(self, step: StepOutput, *, observation="", force_submission: bool = False,reward=1) -> StepOutput:
        """Check if there was a submission in the observation and handle it.
        We add reward calculation logic here as well.
        ...
        Args:
            ...
            reward: If the agent exit due to formatting error or other errors, we directly pass reward=0 to avoid recalculation.
        ...
        if is_submission or force_submission:
            ...
            try:
                ...
                return step
            if  self.config.reward and reward==1:
                reward_calculation_start = time.time()
                self._env.apply_test_patch()
                reward,f2p,p2p,output=self._env._calculate_reward()
                # reward,p2p,f2p,output=0,{},{},''
                reward_calculation_end = time.time()
                self.time_records['reward_calculation']={'start':reward_calculation_start,\
                'end':reward_calculation_end,'calculation_time':reward_calculation_end - reward_calculation_start}
    ...
    def handle_action(self, step: StepOutput) -> StepOutput:
        ...
        We implement the step limit and timeout handling here.
        ...
        try:
            env_communication_start = time.time()
            ...
            self.time_records[self.model.n_calls].update({'env_communication_start':env_communication_start,\
            'env_communication_end':env_communication_end,'execution_time':env_communication_end - env_communication_start})
            ...
            env_communication_end = time.time()
            self.time_records[self.model.n_calls].update({'env_communication_start':env_communication_start,\
            'env_communication_end':env_communication_end,'execution_time':env_communication_end - env_communication_start,"timeout":True})
            self.model.n_calls+=1
        ...
        except ExceedingTotalExecutionTimes:
            try:
                self._env.interrupt_session()
            except Exception as f:
                self.logger.exception("Failed to interrupt session after command timeout: %s", f, exc_info=True)
    ...
    def forward(self, history: list[dict[str, str]]) -> StepOutput:
        ...
        if self.running_time > self.config.total_time_limit:
            raise _TotalRunningTimeExceeded()
        ...
        try:
            ...
            model_response_end = time.time()
            self.time_records[self.model.n_calls]={'model_response_start':model_response_start,\
            'model_response_end':model_response_end,'response_time':model_response_end - model_response_start}
    ...
    def forward_with_handling(self, history: list[dict[str, str]]) -> StepOutput:
        ...
        while n_format_fails < self.max_requeries:
            ...
            except _TotalRunningTimeExceeded:
...
from .agent_sandbox import DefaultAgent as SandboxAgent
...
def get_agent_from_config(config: AgentConfig) -> AbstractAgent:
    ...
    if config.type == "default":
        ...
        return EmptyAgent.from_config(config)
    else:

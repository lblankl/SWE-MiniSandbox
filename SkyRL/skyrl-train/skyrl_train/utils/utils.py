...
def initialize_ray(cfg: DictConfig):
    ...
    print("[DEBUG] initialize_ray: start", flush=True)

    env_vars = prepare_runtime_environment(cfg)

    print("[DEBUG] initialize_ray: before ray.init", flush=True)
    ...
    try:
        ...
        import traceback
        print("[DEBUG] initialize_ray: ray.init exception:", e, flush=True)
        traceback.print_exc()
        raise
    print("[DEBUG] initialize_ray: after ray.init", flush=True)

    print("[DEBUG] initialize_ray: before sync_registries", flush=True)
    sync_registries()
    print("[DEBUG] initialize_ray: after sync_registries", flush=True)

    print("[DEBUG] initialize_ray: done", flush=True)
